...
public:
	...
	void onAppInitialized_impl() override
	{
		...
		uint32_t msdfWidth = 32;
		uint32_t msdfHeight = 32;
		...
		auto image = asset::ICPUImage::create(std::move(imgParams));
		auto texelBuffer = core::make_smart_refctd_ptr<asset::ICPUBuffer>(rowLength * msdfHeight);

		auto texelBufferPtr = reinterpret_cast<char*>(texelBuffer->getPointer());
		for (int y = 0; y < msdfMap.height(); ++y)
		{
			for (int x = 0; x < msdfMap.width(); ++x)
			{
				auto pixel = msdfMap(x, y);
		...
		image->setBufferAndRegions(core::smart_refctd_ptr(texelBuffer), regions);
		...
		viewParams.format = imgParams.format;
		...
		auto imgView = asset::ICPUImageView::create(std::move(viewParams));
		asset::IAssetWriter::SAssetWriteParams writeParams(imgView.get());
		assetManager->writeAsset("msdfoutput.png", writeParams);
		//msdfgen::savePng(msdfMap, "output.png");
